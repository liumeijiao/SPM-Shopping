                    	...
                    	<c:forEach items="${categories }" var="category" varStatus="vs">
                			<c:if test="${vs.index !=0}">
                				<c:if test="${vs.index != fn:length(categories)-1 }">
                					<li>
                						<a href="${pageContext.request.contextPath }/ProductsServlet?op=byCid&cid=${category.cid}">${category.cname}</a>
                					</li>
                				</c:if>
                			</c:if>
                			<c:if test="${vs.index==0 }">
	        	...
	        	<table width="680px" cellspacing="0" cellpadding="5">
	                      ...
	                      <c:if test="${!empty user }">
		                      <c:forEach items="${shoppingCar}" var="item">
									<tr>
			                        	<td><img src="${pageContext.request.contextPath }/${item.product.imgurl }" style="width: 200px;height: 180px" alt="" /></td>
                    ...
                    <c:if test="${!empty shoppingCar}"><p><a href="${pageContext.request.contextPath }/placeOrder.jsp">立即购买</a></p></c:if>
